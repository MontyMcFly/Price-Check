  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: existingProduct } = await supabase
        .from('products').select('id').ilike('name', form.productName.trim()).limit(1);
      ...
      const { data: existingStore } = await supabase
        .from('stores').select('id').ilike('name', form.store.trim()).limit(1);

      if (existingStore && existingStore.length > 0) {
        storeId = existingStore[0].id;
      } else {
        const { data: newStore, error: storeErr } = await supabase
          .from('stores').insert({ name: form.store.trim(), location: form.branch.trim() || null })
          .select().single();
      ...
      if (addToList && user) {
        const { data: listExisting } = await supabase.from('shopping_list').select('id')
          .eq('product_id', productId).eq('user_id', user.id).eq('status', 'pending').limit(1);
        if (!listExisting || listExisting.length === 0) {
          await supabase.from('shopping_list').insert({
            product_id: productId, user_id: user.id,
            quantity: parseInt(form.units) || 1, status: 'pending',
          });
        }
      }

      setSuccess('¡Producto y precio guardados correctamente!');
      setForm({ productName: '', category: '', units: '1', contentAmount: '', contentUnit: 'ml', date: today(), store: '', branch: '', price: '' });
      setAddToList(false);
      onSuccess?.();
    } catch (err: unknown) {
    ...
    <form onSubmit={handleSubmit} className={styles.form}>
      <div className={styles.section}>
        ...
        <div className={styles.field}>
          ...
          <input type="text" value={form.productName} onChange={e => set('productName', e.target.value)} required placeholder="ej. Coca Cola" className={styles.input} />
        </div>
        <div className={styles.field}>
      ...
      <div className={styles.section}>
        ...
        <div className={styles.field}>
          ...
          <input type="text" value={form.store} onChange={e => set('store', e.target.value)} required placeholder="ej. Walmart" className={styles.input} />
        </div>
        <div className={styles.field}>
          <label className={styles.label}>Sucursal <span className={styles.optional}>(opcional)</span></label>
          <input type="text" value={form.branch} onChange={e => set('branch', e.target.value)} placeholder="ej. Centro" className={styles.input} />
        </div>
        <div className={styles.field}>
